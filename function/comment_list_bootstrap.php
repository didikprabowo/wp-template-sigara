...
?>
        <<?php echo $tag; ?> id="comment-<?php comment_ID(); ?>" <?php comment_class($this->has_children ? 'has-children media' : ' media'); ?>>
            ...
            <div class="media-body card mt-3 p-4" id="div-comment-<?php comment_ID(); ?>">
                <div class="card-header hoverable">
                    <div class="flex-center">
                        <?php if ($args['avatar_size'] != 0) : ?>
                            <a href="<?php echo get_comment_author_url(); ?>" class="media-object float-left mr-3">
                    ...
                    <div class="comment-metadata flex-center">
                        <a class="hidden-xs-down" href="<?php echo esc_url(get_comment_link($comment->comment_ID, $args)); ?>">
                            <time class="flex small btn-sm btn btn-outline-secondary chip" datetime="<?php comment_time('c'); ?>">
                                <?php comment_date() ?>,
                                <?php comment_time() ?>
                            </time>
                        </a>
                        <ul class="list-inline">
                            <?php edit_comment_link(__('Edit'), '<li class="mt-1 edit-link list-inline-item btn btn-sm btn-outline-secondary chip">', '</li>'); ?>
                            <?php
                            comment_reply_link(array_merge($args, array(
                                'add_below' => 'div-comment',
                                'depth'     => $depth,
                                'max_depth' => $args['max_depth'],
                                'before'    => '<li class="mt-1 reply-link list-inline-item btn btn-sm btn-outline-secondary chip">',
                                'after'     => '</li>'
                            )));
                            ?>
                        </ul>
